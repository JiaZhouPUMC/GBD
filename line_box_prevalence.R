...
ggplot() +
  ...
  geom_errorbar(data = EAPC_data %>% filter(metric_name == "Rate"), 
                ...
                width = 0.2) +  # 添加误差条
  labs(title = "A Total Number of Prevalence and Age-standardized Rates (Per 100,000 Persons) by Sex",
       x = "Year",
       y = "Number of Prevalence (million)",  # 修改Y轴标签
       color = "sex (rate)",
       fill = "sex (number)") +
  ...
  scale_y_continuous(sec.axis = sec_axis(~ . / scale_factor, name = "Age-standardized Prevalence Rate (per 100,000 persons)")) +  # 添加第二个Y轴
