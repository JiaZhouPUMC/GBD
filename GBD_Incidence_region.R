...
EC_1990 <- EC_1990[,c(4,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
...
EC_2021 <- EC_2021[,c(4,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
...
ASR_1990 <- ASR_1990[,c(4,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
...
ASR_2021 <- ASR_2021[,c(4,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
...
EAPC <- subset(EC, EC$age_name=='Age-standardized' & 
                    ...
                    EC$measure_name=='Incidence'&
                    EC$sex_name=="Both")
EAPC <- EAPC[,c(4,13,14)] ##获取地区、年份以及对应的数值



country <- EC_1990$location
EAPC_cal <- data.frame('location_name'=country,EAPC=rep(0,times=10),UCI=rep(0,times=10),LCI=rep(0,times=10)) 
for (i in 1:10){  ###总共10个地区，所以循环10次
...
### 数据整合
EC_1990 <- EC_1990[,c(1,2)]  ###取地区和整合好的变量
colnames(EC_1990)=c('location_name','NUM_1990')
ASR_1990 <- ASR_1990[,c(1,2)]
colnames(ASR_1990)=c('location_name','ASR_1990')
EC_2021 <- EC_2021[,c(1,2)]
colnames(EC_2021)=c('location_name','NUM_2021')
ASR_2021 <- ASR_2021[,c(1,2)]
colnames(ASR_2021)=c('location_name','ASR_2021')
EAPC_cal <- EAPC_cal[,c(1,5)]
colnames(EAPC_cal)=c('location_name','EAPC')
...
write.csv(Incidence,'Results for incidence_region.csv')
...
order <- read.csv('order.csv', header = FALSE)
...
# 选择 global 数据
EC_global <- subset(EC, location_name == 'Global')

# 定义计算EAPC及置信区间的函数
calculate_eapc_ci <- function(data) {
  data <- data[data$age_name == 'All ages' & data$metric_name == 'Rate' & data$measure_name == 'Incidence', ]
  
  model <- lm(log(val) ~ year, data = data)
  ...
  lower_ci <- (exp(slope - 1.96 * se) - 1) * 100
  upper_ci <- (exp(slope + 1.96 * se) - 1) * 100
  
  return(sprintf("%.2f (%.2f-%.2f)", eapc, lower_ci, upper_ci))
}
...
EC_1990 <- EC_1990[,c(6,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
EC_1990$val <- round(EC_1990$val,1)  ###取整
EC_1990$lower <- round(EC_1990$lower,1)###取整
EC_1990$upper <- round(EC_1990$upper,1) ###取整
EC_1990$Num_1990 <- paste(EC_1990$lower,EC_1990$upper,sep = '-') ## 用-连接95%UI上下数值
...
EC_2021 <- EC_2021[,c(6,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
EC_2021$val <- round(EC_2021$val,1)  ###取整
EC_2021$lower <- round(EC_2021$lower,1)###取整
EC_2021$upper <- round(EC_2021$upper,1) ###取整
EC_2021$Num_2021 <- paste(EC_2021$lower,EC_2021$upper,sep = '-') ## 用-连接95%UI上下数值
...
ASR_1990 <- ASR_1990[,c(6,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
ASR_1990$val <- round(ASR_1990$val,1)  ###取整
ASR_1990$lower <- round(ASR_1990$lower,1)###取整
ASR_1990$upper <- round(ASR_1990$upper,1) ###取整
ASR_1990$ASR_1990 <- paste(ASR_1990$lower,ASR_1990$upper,sep = '-') ## 用-连接95%UI上下数值
...
ASR_2021 <- ASR_2021[,c(6,14,15,16)]  ### 只取需要的变量：地区以及对应的数值
ASR_2021$val <- round(ASR_2021$val,1)  ###取整
ASR_2021$lower <- round(ASR_2021$lower,1)###取整
ASR_2021$upper <- round(ASR_2021$upper,1) ###取整
ASR_2021$ASR_2021 <- paste(ASR_2021$lower,ASR_2021$upper,sep = '-') ## 用-连接95%UI上下数值
...
EAPC <- subset(EC, EC$age_name=='Age-standardized' & 
                 ...
                 EC$measure_name=='Incidence'&
                 EC$location_name=="Global")
EAPC <- EAPC[,c(6,13,14)] ##获取地区、年份以及对应的数值



sexname <- EC_1990$sex_name
EAPC_cal <- data.frame('sex_name'=sexname,EAPC=rep(0,times=3),UCI=rep(0,times=3),LCI=rep(0,times=3)) 
for (i in 1:3){  ###总共10个地区，所以循环10次
  sex_cal <- as.character(EAPC_cal[i,1]) ### 依次取对应的地区
  a <- subset(EAPC, EAPC$'sex_name'==sex_cal)  ##取对应地区的数据子集
  a$y <- log(a$val)  ##根据EAPC计算方法计算y值
  mod_simp_reg<-lm(y~year,data=a) ##根据EAPC计算方法做线性回归方程
  estimate <- (exp(summary(mod_simp_reg)[["coefficients"]][2,1])-1)*100 ##根据EAPC计算方法取方程beta值来计算EAPC
  low <- (exp(summary(mod_simp_reg)[["coefficients"]][2,1]-1.96*summary(mod_simp_reg)[["coefficients"]][2,2])-1)*100
...
write.csv(Incidence,'Results for incidence_sex.csv')
