...
# 设置地区顺序
EC$location_name <- factor(EC$location_name, levels = order$V1, ordered = TRUE)
  ...
  # 对Rate数据进行缩放以适应第二个Y轴
  scale_factor <- max(EAPC_data$val[EAPC_data$metric_name == "Number"], na.rm = TRUE) / 
    max(EAPC_data$val[EAPC_data$metric_name == "Rate"], na.rm = TRUE)
  ...
  # 筛选数据
  EAPC_data <- EC %>%
    filter(sex_name %in% c("Both", "Male", "Female") & 
             location_name == 'Global' & 
             age_name == 'All ages' & 
             metric_name %in% c("Number", "Rate")) %>%
    select(year, sex_name, val, upper, lower, metric_name)
  ...
  ggplot() +
    ...
    geom_errorbar(data = EAPC_data %>% filter(metric_name == "Rate"), 
                  ...
                  width = 0.2) +  # 添加误差条
    labs(title = "A Total Number of Incident Cases and Age-standardized Incidence Rates (Per 100,000 Persons) by Sex",
         x = "Year",
         y = "Number of Incidents (in millions)",  # 修改Y轴标签
         color = "sex (rate)",
         fill = "sex (number)") +
    ...
    scale_y_continuous(sec.axis = sec_axis(~ . / scale_factor, name = "Age-standardized Incidence Rate (per 100,000 persons)")) +  # 添加第二个Y轴
